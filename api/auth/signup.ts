...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        const { data, error } = await supabaseAuthClient.auth.signUp({
            ...
            options: {
                data: {
                    name, // Storing extra metadata like name
                }
            }
        });
        ...
        // Sometimes Supabase requires email verification, so session might be null
        return res.status(200).json({ user: data.user, message: 'Signup requires verification' });
    } catch (err: any) {
